        ...
        return r.goToSlide = function (e, i) {
			...
			//alert('i'+i);
							$.ajax({
									...
									type: "POST",
									url: "/include/common/ajax_retail_iteam.asp",
									data: "seq="+ e,
									cache: false,
			...
			{
				var pagenum = parseInt(url_value_change2(total_url,'gotopage')) + parseInt('1');
				//var pagenum = eval(pagenum+i);	
				change_nowurl('gotopage',pagenum);
			}
...
var total_url = window.location.href; //현재링크
var query_url = window.location.search; //현재페이지에넘어온값들
var hostname = window.location.hostname;//http://도메인
var host = window.location.host; //도메인
var port = window.location.port; //포트
var pathname = window.location.pathname; //파일경로
var protocoal = window.location.protocol; //프로토콜

//alert(hostname+" , "+total_url+" , "+host+" , "+port+" , "+pathname+" , "+query_url+" , "+protocoal);


function change_nowurl(change_var,change_val,mode){
  total_url = url_value_change(total_url,change_var,change_val);
  if (mode == "return")
   return total_url;
  else
...
function url_value_change(url_str,pre_str,next_str,mode){
  url_str = url_str.split("#").join("")
  if(url_str.indexOf(pre_str) > -1){
   url_right = url_str.substring(url_str.indexOf(pre_str));
   joint_str = url_str.substring(url_str.indexOf(pre_str)-1,url_str.indexOf(pre_str));
   if(url_right.indexOf("&") > -1)
    url_right = joint_str+pre_str+"="+next_str+url_right.substring(url_right.indexOf("&"));
   else
    url_right = joint_str+pre_str+"="+next_str;
   url_str = url_str.substring(0,url_str.indexOf(pre_str)-1) + url_right;
  }
  else{
   if(url_str.indexOf("?") > -1)
    url_str = url_str + "&"+pre_str+"="+next_str;
   else
    url_str = url_str + "?"+pre_str+"="+next_str;
  }
  if (mode == "link")
 ...
 function url_value_change2(url_str,pre_str){
